...
func (_ *DHCPOptions) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *DHCPOptions) error {
	if a == nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().CreateDhcpOptions(request)
		if err != nil {
	...
	return t.Cloud.CloudProvider().(awsup.AWSCloud).AddAWSTags(*e.ID,
		t.Cloud.CloudProvider().(awsup.AWSCloud).BuildTags(e.Name))
}
