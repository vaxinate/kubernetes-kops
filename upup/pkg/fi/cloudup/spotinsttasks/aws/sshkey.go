...
func (_ *SSHKey) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *SSHKey) error {
	if a == nil {
		return e.createKeypair(t.Cloud.CloudProvider().(awsup.AWSCloud))
	}

	// No tags on SSH public key
	return nil //return output.Cloud.CloudProvider().(awsup.AWSCloud).AddAWSTags(cloud.Tags(), v, "vpc")
}
