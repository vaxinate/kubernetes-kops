...
func (_ *SecurityGroup) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *SecurityGroup) error {
	...
	if a == nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().CreateSecurityGroup(request)
		if err != nil {
	...
	return t.Cloud.CloudProvider().(awsup.AWSCloud).AddAWSTags(*e.ID, t.Cloud.CloudProvider().(awsup.AWSCloud).BuildTags(e.Name))
}
...
func (d *deleteSecurityGroupRule) Delete(t fi.Target) error {
	...
	target, ok := t.(*spotinst.SpotinstAPITarget)
	if !ok {
	...
	if d.egress {
		...
		_, err := target.Cloud.CloudProvider().(awsup.AWSCloud).EC2().RevokeSecurityGroupEgress(request)
		if err != nil {
		...
		_, err := target.Cloud.CloudProvider().(awsup.AWSCloud).EC2().RevokeSecurityGroupIngress(request)
		if err != nil {
