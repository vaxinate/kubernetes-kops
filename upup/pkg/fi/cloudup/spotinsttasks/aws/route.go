...
func (_ *Route) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *Route) error {
	if a == nil {
		...
		if e.InternetGateway == nil && e.NatGateway == nil {
			...
			if err := e.NatGateway.waitAvailable(t.Cloud.CloudProvider().(awsup.AWSCloud)); err != nil {
				return err
			}
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().CreateRoute(request)
		if err != nil {
		...
		_, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().ReplaceRoute(request)
		if err != nil {
