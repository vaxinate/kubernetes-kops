...
func (e *EBSVolume) FindResourceID(c fi.Cloud) (*string, error) {
	actual, err := e.find(c.(*spotinst.SpotinstCloud).CloudProvider().(awsup.AWSCloud))
	if err != nil {
...
func (e *EBSVolume) Find(c *fi.Context) (*EBSVolume, error) {
	actual, err := e.find(c.Cloud.(*spotinst.SpotinstCloud).CloudProvider().(awsup.AWSCloud))
...
func (e *EBSVolume) Run(c *fi.Context) error {
	c.Cloud.(*spotinst.SpotinstCloud).CloudProvider().(awsup.AWSCloud).AddTags(e.Name, e.Tags)
...
func (_ *EBSVolume) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *EBSVolume) error {
	if a == nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().CreateVolume(request)
		if err != nil {
	...
	return t.Cloud.CloudProvider().(awsup.AWSCloud).AddAWSTags(*e.ID, e.Tags)
}
