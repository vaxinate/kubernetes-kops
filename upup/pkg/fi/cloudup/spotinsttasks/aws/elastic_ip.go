...
func (e *ElasticIP) Find(c *fi.Context) (*ElasticIP, error) {
	return e.find(c.Cloud.(*spotinst.SpotinstCloud).CloudProvider().(awsup.AWSCloud))
}
...
func (_ *ElasticIP) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *ElasticIP) error {
	var publicIp *string
	var eipId *string
	...
	if a == nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().AllocateAddress(request)
		if err != nil {
	...
	if e.TagOnSubnet != nil {
		...
		err := t.Cloud.CloudProvider().(awsup.AWSCloud).AddAWSTags(*e.TagOnSubnet.ID, tags)
		if err != nil {
