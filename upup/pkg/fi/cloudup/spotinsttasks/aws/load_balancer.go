...
func (e *LoadBalancer) Find(c *fi.Context) (*LoadBalancer, error) {
	cloud := c.Cloud.(*spotinst.SpotinstCloud).CloudProvider().(awsup.AWSCloud)
...
func (e *LoadBalancer) FindIPAddress(context *fi.Context) (*string, error) {
	cloud := context.Cloud.(*spotinst.SpotinstCloud).CloudProvider().(awsup.AWSCloud)
...
func (_ *LoadBalancer) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *LoadBalancer) error {
	...
	if a == nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).ELB().CreateLoadBalancer(request)
		if err != nil {
		...
		lb, err := findLoadBalancerByLoadBalancerName(t.Cloud.CloudProvider().(awsup.AWSCloud), loadBalancerName)
		if err != nil {
			return err
		}
		if lb == nil {
		...
		if changes.Listeners != nil {
			...
			_, err := t.Cloud.CloudProvider().(awsup.AWSCloud).ELB().CreateLoadBalancerListeners(request)
			if err != nil {
	...
	if err := t.Target.(*awsup.AWSAPITarget).AddELBTags(loadBalancerName,
		t.Cloud.CloudProvider().(awsup.AWSCloud).BuildTags(e.Name)); err != nil {
		return err
	}
	...
	if changes.HealthCheck != nil && e.HealthCheck != nil {
		...
		_, err := t.Cloud.CloudProvider().(awsup.AWSCloud).ELB().ConfigureHealthCheck(request)
		if err != nil {
