...
func (_ *RouteTableAssociation) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *RouteTableAssociation) error {
	if a == nil {
		...
		existing, err := findExistingRouteTableForSubnet(t.Cloud.CloudProvider().(awsup.AWSCloud), e.Subnet)
		if err != nil {
		...
		if existing != nil {
			for _, a := range existing.Associations {
				...
				_, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().DisassociateRouteTable(request)
				if err != nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().AssociateRouteTable(request)
		if err != nil {
