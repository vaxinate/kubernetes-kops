...
func (_ *VPC) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *VPC) error {
	...
	if a == nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().CreateVpc(request)
		if err != nil {
	...
	if changes.EnableDNSSupport != nil {
		...
		_, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().ModifyVpcAttribute(request)
		if err != nil {
