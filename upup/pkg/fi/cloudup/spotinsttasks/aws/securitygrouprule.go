...
func (_ *SecurityGroupRule) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *SecurityGroupRule) error {
	...
	if a == nil {
		...
		if fi.BoolValue(e.Egress) {
			...
			_, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().AuthorizeSecurityGroupEgress(request)
			if err != nil {
			...
			_, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().AuthorizeSecurityGroupIngress(request)
			if err != nil {
