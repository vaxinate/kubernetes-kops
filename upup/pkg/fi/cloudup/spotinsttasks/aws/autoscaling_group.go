...
func findAutoscalingGroup(cloud *spotinst.SpotinstCloud, name string) (*aws.Group, error) {
	output, err := cloud.Service.CloudProviderAWS().List(context.TODO(), nil)
	if err != nil {
...
func (e *AutoscalingGroup) Find(c *fi.Context) (*AutoscalingGroup, error) {
	...
	// Launch Specification.
	{
		...
		// Image.
		{
			image, err := cloud.CloudProvider().(awsup.AWSCloud).ResolveImage(*lc.ImageID)
			if err != nil {
...
func (_ *AutoscalingGroup) Render(t *spotinst.SpotinstAPITarget, a, e, changes *AutoscalingGroup) error {
	if a == nil {
		...
		// Strategy.
		{
			group.Strategy.SetAvailabilityVsCost(spotinstsdk.String("balanced"))
			...
			group.Strategy.SetRisk(spotinstsdk.Float64(risk))
		}

		// Compute.
		{
			group.Compute.SetProduct(spotinstsdk.String(aws.ProductLinuxUnix.String()))
			...
			// Launch Specification.
			{
				...
				// Image ID.
				{
					image, err := t.Cloud.CloudProvider().(awsup.AWSCloud).ResolveImage(*e.ImageID)
					if err != nil {
				...
				{
					tagsMap := e.buildTags()
					tags := make([]*aws.Tag, 0, len(tagsMap))
	...
	readyLoop:
		for {
			...
			out, err := t.Cloud.Service.CloudProviderAWS().Create(context.Background(), input)
			if err == nil {
				e.ID = out.Group.ID
				break
			}
		...
		actual, err := findAutoscalingGroup(t.Cloud, *e.Name)
		if err != nil {
			glog.Errorf("Unable to find group: %s, error: %s", *e.Name, err)
			return err
		}
		glog.V(2).Infof("Found group ID: %s -> %s", *e.Name, *actual.ID)
		...
		if group.Compute == nil && group.Capacity == nil {
			glog.V(2).Infof("No changes detected in group: %s", *group.ID)
			return nil
		}

		glog.V(2).Infof("Updating group: %s, group: %s", *group.ID, stringutil.Stringify(group))
...
func (e *AutoscalingGroup) buildTags() map[string]string {
...
func (e *AutoscalingGroup) buildRootDevice(cloud *spotinst.SpotinstCloud) (map[string]*BlockDeviceMapping, error) {
	imageID := fi.StringValue(e.ImageID)
	image, err := cloud.CloudProvider().(awsup.AWSCloud).ResolveImage(imageID)
	if err != nil {
