...
func (_ *NatGateway) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *NatGateway) error {
	// New NGW
	...
	if a == nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().CreateNatGateway(request)
		if err != nil {
	...
	err := t.Cloud.CloudProvider().(awsup.AWSCloud).AddAWSTags(*e.Subnet.ID, tags)
	if err != nil {
	...
	if fi.BoolValue(e.Shared) {
		...
		err = t.Cloud.CloudProvider().(awsup.AWSCloud).AddAWSTags(fi.StringValue(e.AssociatedRouteTable.ID), tags)
		if err != nil {
