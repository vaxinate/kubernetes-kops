...
func (_ *DNSZone) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *DNSZone) error {
	...
	if a == nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).Route53().CreateHostedZone(request)
		if err != nil {
		...
		if changes.PrivateVPC != nil {
			...
			_, err := t.Cloud.CloudProvider().(awsup.AWSCloud).Route53().AssociateVPCWithHostedZone(request)
			if err != nil {
