...
func (_ *IAMRole) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *IAMRole) error {
	...
	if a == nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).IAM().CreateRole(request)
		if err != nil {
		...
		if changes.RolePolicyDocument != nil {
			...
			_, err = t.Cloud.CloudProvider().(awsup.AWSCloud).IAM().UpdateAssumeRolePolicy(request)
			if err != nil {
