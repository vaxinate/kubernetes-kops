...
func (_ *IAMInstanceProfile) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *IAMInstanceProfile) error {
	if a == nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).IAM().CreateInstanceProfile(request)
		if err != nil {
		...
		for {
			...
			ip, err := findIAMInstanceProfile(t.Cloud.CloudProvider().(awsup.AWSCloud), *e.Name)
			if err != nil {
