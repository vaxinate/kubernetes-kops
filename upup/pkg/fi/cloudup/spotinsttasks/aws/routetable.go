...
func (_ *RouteTable) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *RouteTable) error {
	if a == nil {
		vpcID := e.VPC.ID
		if vpcID == nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().CreateRouteTable(request)
		if err != nil {
