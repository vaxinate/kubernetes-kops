...
func (_ *LoadBalancer) modifyLoadBalancerAttributes(t *spotinst.SpotinstAPITarget, a, e, changes *LoadBalancer) error {
	...
	_, err := t.Cloud.CloudProvider().(awsup.AWSCloud).ELB().ModifyLoadBalancerAttributes(request)
	if err != nil {
