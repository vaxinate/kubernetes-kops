...
func (_ *IAMRolePolicy) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *IAMRolePolicy) error {
	...
	if policy == "" {
		...
		_, err = t.Cloud.CloudProvider().(awsup.AWSCloud).IAM().DeleteRolePolicy(request)
		if err != nil {
	...
	if doPut {
		...
		_, err = t.Cloud.CloudProvider().(awsup.AWSCloud).IAM().PutRolePolicy(request)
		if err != nil {
