...
func (_ *InternetGateway) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *InternetGateway) error {
	shared := fi.BoolValue(e.Shared)
	if shared {
	...
	if a == nil {
		...
		response, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().CreateInternetGateway(request)
		if err != nil {
	...
	if a == nil || (changes != nil && changes.VPC != nil) {
		...
		_, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().AttachInternetGateway(attachRequest)
		if err != nil {
			return fmt.Errorf("error attaching InternetGateway to VPC: %v", err)
		}
	}

	tags := t.Cloud.CloudProvider().(awsup.AWSCloud).BuildTags(e.Name)
	if shared {
		// Don't tag shared resources
		tags = nil
	}
	return t.Cloud.CloudProvider().(awsup.AWSCloud).AddAWSTags(*e.ID, tags)
}
