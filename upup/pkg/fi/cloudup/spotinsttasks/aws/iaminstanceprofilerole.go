...
func (_ *IAMInstanceProfileRole) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *IAMInstanceProfileRole) error {
	if a == nil {
		...
		_, err := t.Cloud.CloudProvider().(awsup.AWSCloud).IAM().AddRoleToInstanceProfile(request)
		if err != nil {
