...
func (_ *VPCDHCPOptionsAssociation) RenderAWS(t *spotinst.SpotinstAPITarget, a, e, changes *VPCDHCPOptionsAssociation) error {
	if changes.DHCPOptions != nil {
		...
		_, err := t.Cloud.CloudProvider().(awsup.AWSCloud).EC2().AssociateDhcpOptions(request)
		if err != nil {
