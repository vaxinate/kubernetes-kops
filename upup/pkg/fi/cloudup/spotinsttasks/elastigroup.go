...
func (e *Elastigroup) Find(c *fi.Context) (*Elastigroup, error) {
	awsCloud := c.Cloud.(awsup.AWSCloud)
	spotinstCloud := c.Cloud.(spotinst.Cloud)

	group, err := e.find(spotinstCloud.Spotinst(), *e.Name)
	if err != nil {
		return nil, err
	}
	if group == nil {
	...
	// Launch Specification.
	{
		...
		// Image.
		{
			image, err := resolveImage(awsCloud, fi.StringValue(lc.ImageID))
			if err != nil {
				return nil, err
			}
...
func (eg *Elastigroup) RenderAWS(t *spotinst.AWSAPITarget, a, e, changes *Elastigroup) error {
	return eg.createOrUpdate(t.Cloud, a, e, changes)
}

func (eg *Elastigroup) createOrUpdate(cloud fi.Cloud, a, e, changes *Elastigroup) error {
	if a == nil {
...
func (_ *Elastigroup) create(cloud fi.Cloud, a, e, changes *Elastigroup) error {
	...
	// Compute.
	{
		...
		// Launch Specification.
		{
			...
			// Block device mappings.
			{
				rootDevices, err := e.buildRootDevice(awsCloud)
				if err != nil {
					return err
				}
			...
			// Image ID.
			{
				image, err := resolveImage(awsCloud, fi.StringValue(e.ImageID))
				if err != nil {
					return err
				}
			...
			// Load balancer.
			{
				if e.LoadBalancer != nil {
					elb, err := awstasks.FindLoadBalancerByNameTag(awsCloud, fi.StringValue(e.LoadBalancer.Name))
					if err != nil {
						return err
					}
					if elb != nil {
...
readyLoop:
	for {
		...
		eg, err := spotinst.NewElastigroup(awsCloud.ProviderID(), group)
		if err != nil {
			return err
		}

		// Create the Elastigroup.
		id, err := spotinstCloud.Spotinst().Create(context.Background(), eg)
		if err == nil {
			e.ID = fi.String(id)
			break
		}
...
func (_ *Elastigroup) update(cloud fi.Cloud, a, e, changes *Elastigroup) error {
	glog.V(2).Infof("Updating elastigroup %q", *e.Name)

	awsCloud := cloud.(awsup.AWSCloud)
	spotinstCloud := cloud.(spotinst.Cloud)

	actual, err := e.find(spotinstCloud.Spotinst(), *e.Name)
	if err != nil {
	...
	if err := spotinstCloud.Spotinst().Update(context.Background(), eg); err != nil {
...
func (_ *Elastigroup) RenderTerraform(t *terraform.TerraformTarget, a, e, changes *Elastigroup) error {
	...
	tf := &terraformElastigroup{
		...
		Description: e.Name,
		Product:     e.Product,
		Region:      fi.String(awsCloud.Region()),
